...
const processSingleImage = async (image) => {
  ...
  try {
    const generateHighResFullImageAsWebp = await sharp(image_url)
      .resize({
        width: set_image_width_as_max_width ? 1500 : image_metadata.width,
      })
      .toFormat("webp")
      .webp({ quality: 80 })
  ...
  if (is_thumbnail_required) {
    ...
    try {
      const generateHighResThumbImageAsWebp = await sharp(
        imageToReturn.image_url.full.high_res
      )
        .resize({
          width: 250,
          height: 250,
    ...
    try {
      const generateLowResThumbImageAsWebp = await sharp(
        imageToReturn.image_url.full.high_res
      )
        .resize({
          width: 250,
    ...
    imageToReturn.image_url.thumbnail.high_res =
      imageToReturn.image_url.full.high_res;
    imageToReturn.image_url.thumbnail.low_res =
      imageToReturn.image_url.full.low_res;
  }

  // Get the metadata of the optimized image
  let optimized_file_metadata = await sharp(
    imageToReturn.image_url.full.high_res
  ).metadata();
...
module.exports.saveMediaFile = async (req, res) => {
  ...
  if (isFileTypeImage) {
    ...
    new_media.name.temp = processImage.data.name.current.replace(/\s+/g, "");
    new_media.name.original = processImage.data.name.original.replace(
      /\s+/g,
      ""
    );
    new_media.name.current = processImage.data.name.current.replace(/\s+/g, "");
    ...
    new_media.image_url.full.high_res =
      processImage.data.image_url.full.high_res;
    new_media.image_url.full.low_res = processImage.data.image_url.full.low_res;
    new_media.image_url.thumbnail.high_res =
      processImage.data.image_url.thumbnail.high_res;
    new_media.image_url.thumbnail.low_res =
      processImage.data.image_url.thumbnail.low_res;
  } else if (isFileTypeVideo) {
    processVideo = await processSingleVideo(req.file);
    // You can add similar space trimming logic here if needed for video filenames
  } else if (isFileTypeDocument) {
    processDocument = await processSingleDocument(req.file);
    if (processDocument.status === true) {
      new_media.name.temp = processDocument.data.name.current.replace(
        /\s+/g,
        ""
      );
      new_media.name.original = processDocument.data.name.original.replace(
        /\s+/g,
        ""
      );
      new_media.name.current = processDocument.data.name.current.replace(
        /\s+/g,
        ""
      );
      new_media.extension.original = processDocument.data.extension.original;
    ...
    processAudio = await processSingleAudio(req.file);
    // Add space trimming logic here if audio filenames are handled similarly
  }
...
module.exports.getAllAuthFiles = async (req, res) => {
  ...
  if (
    ...
    decodedToken.role != "TNO" &&
    decodedToken.role != "DC" &&
    decodedToken.role != "DS" &&
    decodedToken.role != "CEO" &&
    decodedToken.role != "AR" &&
    decodedToken.role != "US"
  ) {
    ...
    const getFilesReq = await Media.aggregate([
      {
        $match: {
          is_archived: false,
          media_type: mediaTypesFilter,
          belongs_to_folder: belongsToFilter,
          uploaded_by: createdByFilter,
        },
      },
      {
        $lookup: {
      ...
      },
      {
        $facet: {
          metadata: [
            { $count: "total" },
            { $addFields: { page: parseInt(pageInt) } },
          ],
    ...
    return res.status(STATUS.SUCCESS).json({
      ...
      data: {
        ...
        total_pages: Math.ceil(
          getFilesReq[0].metadata[0].total / parseInt(sizeInt)
        ),
...
module.exports.getThisAuthFile = async (req, res) => {
  ...
  try {
    const file = await Media.findOne(
      { _id: req.params.id, is_archived: false },
      {
        name: 1,
        size: 1,
        extension: 1,
